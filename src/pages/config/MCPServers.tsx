        ...
        <div className="space-y-2">
          {(data?.servers || []).map((s: any, idx: number) => (
            <div key={idx} className="border rounded p-3 flex items-center justify-between dark:border-gray-700 dark:bg-gray-800">
              <div>
                <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{s.base_url}</div>
                <div className="text-xs text-gray-600 dark:text-gray-400">{s.tools.length} tool(s)</div>
              </div>
              <button onClick={() => remove.mutate(s.base_url)} className="btn-danger">Remove</button>
            </div>
          ))}
        </div>
      </div>
